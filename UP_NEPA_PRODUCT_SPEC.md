...
- Lightweight account system (Name + Email + 4-digit PIN, or Google OAuth) — OPay-style instant re-entry
...
### Account system — PIN-based, OPay-style

The goal: signup is a one-time event. After that, returning to the app on the same device is near-instant — just a 4-digit PIN. No repeated email entry, no repeated OAuth popups.

**First-time signup (one screen):**
- Name + Email
- Continue with Google (OAuth) as alternative to email
- Create PIN (4 digits) + Confirm PIN

**Returning user, same device (recognized session):**
- Just enter PIN — no email, no re-auth

**Returning user, new/unrecognized device:**
- Email + PIN (PIN doubles as the cross-device credential)

**Forgot PIN:**
- Reset link sent to email → set new PIN
...
└─────────────────────────────────┘
```
If PINs don't match: inline error "PINs don't match — try again" before allowing Continue.
...
└─────────────────────────────────┘
```
No email. No password. Instant entry.
...
### Why 4 digits:
This PIN protects power-status data, not financial data — the security bar is intentionally low to prioritize speed and familiarity. 4 digits matches the pattern Nigerian users already know from fintech apps (OPay, Kuda, etc.) without feeling like an OTP.

### Implementation — how PIN + session work together:
- Supabase Auth handles the real authentication — `persistSession: true` stores a valid session token securely on the device on first signup
- The PIN is a **separate, lightweight local gate** in front of that session — stored as a hashed value
- On app open: if a valid persisted session exists, prompt only for PIN. Correct PIN → unlock access to the existing session, no new server auth call needed
- Wrong PIN: lock out after 5 attempts, fall back to Email + PIN re-authentication (which re-establishes the session via Supabase Auth)
- The PIN never replaces the Supabase session token — it's a fast local unlock layer in front of it

### Google OAuth users:
Also set a PIN immediately after first Google sign-in. PIN applies uniformly regardless of signup method — keeps the returning-user experience consistent (no repeated OAuth popups).

### Browser fingerprinting (deprecated for identity)
No longer needed as an identity mechanism — the persisted session + PIN combination fully covers session persistence. Fingerprinting is dropped from the spec entirely.
...
**Triggered immediately after first-time signup (not shown to returning users — they go straight from PIN unlock to Home tab).**  
**3 screens maximum.**
...
name              text
email             text unique
pin_hash          text                        -- hashed 4-digit PIN, local unlock + cross-device credential
...
### Week 1–2: Foundation
- [ ] Supabase Auth — email + PIN system + Google OAuth, persisted session with PIN unlock gate
...
*Document version: MVP v1.5*  
*Last updated: June 2026*  
*Product: Up NEPA — Magboro, Ogun State*  
*Key updates in v1.5: Identity system replaced with OPay-style PIN flow — Name + Email + 4-digit PIN (confirm on creation) or Google OAuth at first signup, persisted session unlocked via PIN on return (same device), Email + PIN for new device sign-in, forgot PIN via email reset. Browser fingerprinting removed entirely. Onboarding reduced to 3 screens and only shown to first-time users.*  
*Previous — v1.4: Full account system (Name + Email + Password + Google OAuth), home area change system with progressive cooldown + area_history, Core Product Principle section, notification fatigue cap, coverage health indicator, sharing trigger (banner + settings), community benchmarking in analytics, monthly report cards as Phase 2, explicit weighted confidence formula, prediction reliability ratings, test feeder seed data for Magboro, cascading dropdown with street hints, Explore map with active reporter count*
